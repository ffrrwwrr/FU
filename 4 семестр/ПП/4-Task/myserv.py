...
def run_server(port=53210):
  ...
  cid = 0
  while True:
    client_sock = accept_client_connection(serv_sock, cid)
    t = threading.Thread(target=server_client,
                         args=(client_sock, cid))
    t.start()
    cid += 1

def server_client(client_sock, cid):
  while True:
...
def create_server_socket(serv_port):
...
def accept_client_connection(serv_sock, cid):
